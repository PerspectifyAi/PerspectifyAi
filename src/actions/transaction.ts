...
const serializeAmount = (obj: { amount: { toNumber: () => number } } & Record<string, unknown>) => ({
  ...obj,
...
export async function createTransaction(data: TransactionData): Promise<{ success: boolean; data: unknown }> {
  try {
...
export async function getUserTransactions(query: Record<string, unknown> = {}): Promise<{ success: boolean; data: unknown }> {
  try {
    const { userId } = await auth();
    if (!userId) throw new Error("Unauthorized");
